...
class DiscountRepo
{
    ...
    public function store($data)
    {
        $discount = Discount::query()->create([
            ...
            'expire_at' => $data['expire_at'] ? \Carbon\Carbon::createFromFormat('d-m-Y H:i', $data['expire_at'])->toCarbon() : null,
            'link' => $data['link'],
            'type' => $data['type'],
    ...
    public function getValidDiscountsQuery($type = 'all', $id = null)
    {
        $query = Discount::query()
            ->where('expire_at', '>=', \Carbon\Carbon::now())  // Use English date time (current time in English)
            ->where('type', $type)
            ->whereNull('code');
