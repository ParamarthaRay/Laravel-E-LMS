...
use Illuminate\Http\Request;  // Import Carbon to handle date formatting
...
class TicketController extends Controller
{
    public function index(TicketRepo $repo, Request $request)
    {
        if (auth()->user()->can(Permission::PERMISSION_MANAGE_TICKETS)) {
            // Format the date to Gregorian if needed
            $formattedDate = $request->date ? Carbon::parse($request->date)->format('Y-m-d') : null;

            $tickets = $repo->joinUsers()
                ->searchEmail($request->email)
                ->searchName($request->name)
                ->searchTitle($request->title)
                ->searchDate($formattedDate)  // Pass the formatted date
