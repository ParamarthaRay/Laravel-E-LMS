<div class="col-12 bg-white margin-bottom-15 border-radius-3">
    <p class="box__title">Chapters</p>
    <form action="{{ route('seasons.store', $course->id) }}" method="post" class="padding-30">
        @csrf
        <x-input type="text" name="title" placeholder="Chapter Title" class="text" required />
        <x-input type="text" name="number" placeholder="Chapter Number" class="text" />
        <button type="submit" class="btn btn-webamooz_net">Add</button>
    </form>
    <div class="table__box padding-30">
        <table class="table">
            <thead role="rowgroup">
            <tr role="row" class="title-row">
                <th class="p-r-90">ID</th>
                <th>Chapter Title</th>
                <th>Status</th>
                <th>Confirmation Status</th>
                <th>Actions</th>
            </tr>
            </thead>
            <tbody>
            @foreach($course->seasons as $season)
            <tr role="row" class="">
                <td><a href="">{{ $season->number }}</a></td>
                <td><a href="">{{ $season->title }}</a></td>
                <td class="status">@lang($season->status)</td>
                <td class="confirmation_status">@lang($season->confirmation_status)</td>
                <td>
                    <a href=""  onclick="deleteItem(event, '{{ route('seasons.destroy', $season->id) }}')" class="item-delete mlg-15" title="Delete"></a>
                    @can(\Cyaxaress\RolePermissions\Models\Permission::PERMISSION_MANAGE_COURSES)
                        <a href="" onclick="updateConfirmationStatus(event, '{{ route('seasons.accept', $season->id) }}',
                            'Are you sure you want to confirm this item?', 'Confirmed')"
                           class="item-confirm mlg-15" title="Confirm"></a>
                        <a href="" onclick="updateConfirmationStatus(event, '{{ route('seasons.reject', $season->id) }}',
                            'Are you sure you want to reject this item?', 'Rejected')"
                           class="item-reject mlg-15" title="Reject"></a>
                        @if($season->status == \Cyaxaress\Course\Models\Season::STATUS_OPENED)
                        <a href="" onclick="updateConfirmationStatus(event, '{{ route('seasons.lock', $season->id) }}',
                            'Are you sure you want to lock this item?', 'Locked', 'status')"
                           class="item-lock text-error mlg-15" title="Lock"></a>
                        @else
                        <a href="" onclick="updateConfirmationStatus(event, '{{ route('seasons.unlock', $season->id) }}',
                            'Are you sure you want to unlock this item?', 'Open', 'status')"
                           class="item-lock mlg-15 text-success" title="Unlock"></a>
                        @endif
                    @endcan
                    <a href="{{ route('seasons.edit', $season->id) }}" class="item-edit " title="Edit"></a>
                </td>
            </tr>
            @endforeach
            </tbody>
        </table>
    </div>
</div>
