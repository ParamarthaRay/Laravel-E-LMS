...
function acceptAllLessons(route) {
    if (confirm("Are you sure you want to approve all sessions in this course?")) {
...
function acceptMultiple(route) {
    doMultipleAction(route, "Are you sure you want to approve these rows?", 'patch');
}
...
$('.course__detial .item-delete').on('click', function (e) {
    var confirmDelete = "Are you sure you want to delete this row?";
...
$(document).on('click touchstart', function (e) {
    var searchBox = $('.t-header-search');
    var input = $('.search-input__box');
    if ($(e.target).is(searchBox) || searchBox.has(e.target).length === 1) {
